...
ptBoolFct dimacsFileToBool(char * filePath) {
	...
	if(dimacsFile != NULL) {
		...
		Cudd_RecursiveDeref(_manager, disjunction);
		fclose(dimacsFile);
		return Cudd_BddToAdd(_manager, conjunction);
	} else return NULL;
}

ptBoolFct createLiteral(int lit) {
	DdNode * var;
	if(lit > 0) var = Cudd_addIthVar(_manager, lit);
	else {
		lit = -lit;
		var = Cudd_addIthVar(_manager, lit);
		if(!var) failure("Out of memory (boolFct-add.c, createLiteral).\n");
		var = Cudd_Not(var);
	}
	if(!var) failure("Out of memory (boolFct-add.c, createLiteral).\n");
	Cudd_Ref(var);
...
ptBoolFct createVariable(char * name) {
	if(!name) return NULL;
	ptBoolFct fct = createLiteral(createMapping(name));
	return fct;
}

ptBoolFct createConstant(double value) {
	DdNode * res = Cudd_addConst(_manager, value);
	Cudd_Ref(res);
	return res;
}
...
ptBoolFct addConjunction(ptBoolFct leftFct, ptBoolFct rightFct, byte preserveLeft, byte preserveRight) {
	...
	DdNode * res = Cudd_addApply(_manager, Cudd_addPlus, (DdNode *) leftFct, (DdNode *) rightFct);
	if(!res) failure("Out of memory (boolFct-add.c, addConjunction).\n");
	Cudd_Ref(res);
...
ptBoolFct addDisjunction(ptBoolFct leftFct, ptBoolFct rightFct, byte preserveLeft, byte preserveRight) {
	...
	DdNode * res = Cudd_addApply(_manager,Cudd_addMinimum, (DdNode *) leftFct, (DdNode *) rightFct);
	if(!res) failure("Out of memory (boolFct-add.c, addDisjunction).\n");
	Cudd_Ref(res);
...
ptBoolFct negateBool(ptBoolFct formula) {
	if(!formula) return NULL;
	DdNode * res = Cudd_addApply(_manager,Cudd_addMinus, createConstant(1), (DdNode *) formula);
...
void destroyBool(ptBoolFct formula) {
	if(formula) Cudd_RecursiveDeref(_manager, (DdNode *) formula);
}

byte isLogicZero(ptBoolFct fct) {
	if(fct && (fct == DD_ZERO(_manager))) return 1;
	else return 0;
}

byte isLogicOne(ptBoolFct fct) {
	if(!fct) return 1;
	if(fct && (fct == DD_ONE(_manager))) return 1;
	else return 0;
}
...
ptBoolFct getMaximum(ptBoolFct fct, ptBoolFct fd) {
	ptBoolFct result = Cudd_addFindMax(_manager, fct);
	Cudd_Ref(result);
	return result;
}

ptBoolFct getMinimum(ptBoolFct fct, ptBoolFct fd) {
	ptBoolFct result = Cudd_addFindMin(_manager, fct);
	Cudd_Ref(result);
	return result;
}
