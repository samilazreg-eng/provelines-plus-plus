...
unsigned int processVariables(ptSymTabNode node, ptSymTabNode globalSymTab, ptMTypeNode mTypes, unsigned int iOffset, byte bGlobal) {
	if(node != NULL) {
        ...
        //printf("Variable %s has memSize = %d, bound = %d, iMemSpace = %u\n", node->name, node->memSize, node->bound, iMemSpace);
...
char* getExpTypeName(int type) {
	switch(type) {
        ...
        case E_STMNT_WHEN	   : return "Time delay (E_STMNT_WHEN)";
        case E_STMNT_COST	   : return "Cost increase (E_STMNT_COST)";
        case E_STMNT_QUALITY   : return "Quality loss (E_STMNT_QUALITY)";
        case E_STMNT_NF        : return "Non-functional (E_STMNT_NF)";
...
void printExpressionPrefix(ptExpNode expression, char * prefix) {
    if(!expression) {
        printf("NULL");
    } else {
        printf("%s Expression of type %s\n", getExpTypeName(expression->type), prefix);
        printf("%s sVal = %s\n", prefix, expression->sVal);
        printf("%s iVal = %d\n", prefix, expression->iVal);
        printf("%s lineNb = %d\n", prefix, expression->lineNb);
        char newPrefix[40];
        strcpy(newPrefix, prefix);
        strcat(newPrefix, "\t");
        printf("%s Children[0]:", prefix);
        printExpressionPrefix(expression->children[0], newPrefix);
        printf("\n");
        printf("%s Children[1]:", prefix);
        printExpressionPrefix(expression->children[1], newPrefix);
        printf("\n");
        printf("%s Children[2]:", prefix);
        printExpressionPrefix(expression->children[2], newPrefix);
    }
}

void printExpression(ptExpNode expression) {
    printExpressionPrefix(expression, "");
}
