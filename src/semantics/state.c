...
unsigned int getVarOffset(ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState state, ptStateMask process, unsigned int preOffset, ptExpNode expression) {
    ...
    //printf("computing offset of %s (type %s)\n", expression->sVal, getExpTypeName(expression->type));
	if(expression->type == E_RARG_VAR  || expression->type == E_EXPR_VAR) {
		varOffset = getVarOffset(globalSymTab, mtypes, state, process, preOffset, expression->children[0]);
		...
		if(!expression->children[1])
			varOffset = highOffset;
		else
            varOffset = getVarOffset(globalSymTab, mtypes, state, process, highOffset, expression->children[1]);
        // DEBUG PRINT
        // printf("%s[%d] at offset %u\n", expression->children[0]->sVal, index, varOffset);
	} else if (expression->type == E_VARREF_NAME) {
        if (expression->children[0]) {
            index = eval(globalSymTab, mtypes, state, process, expression->children[0], EVAL_EXPRESSION, NULL);
        }
        if (index >= expression->symTab->bound) {
            printState(mtypes, state, NULL);
            failure("Index %d exceeds bound of variable %s (%d)", index, expression->children[0]->sVal, expression->symTab->bound);
        }
        
		if(expression->symTab->type == T_CHAN) {
			if(expression->symTab->capacity != 0)
                varOffset = preOffset + expression->symTab->memOffset + (expression->symTab->capacity * expression->symTab->memSize + 1) * index;
			else
                varOffset = preOffset + expression->symTab->memOffset + index;
		}
		else {
			varOffset = preOffset + expression->symTab->memOffset + expression->symTab->memSize * index;
        }
        // DEBUG PRINT
        // printf("%s[%d] at offset %u\n", expression->sVal, index, varOffset);
    } else {
	...
	return varOffset; // only to please the Eclipse analyser
}
...
ptState stateCreateInitial(ptSymTabNode globalSymTab, ptMTypeNode mtypes) {
	ptState newState = (ptState) malloc(sizeof(tState));
	if(!newState) failure("Out of memory (creating State).\n");
	newState->nbProcesses = 0;
	newState->mask = (ptStateMask) malloc(sizeof(tStateMask));
...
int stateAddProctype(ptState state, ptSymTabNode proctype, char * cname, ptExpNode cexp, int index) {
	...
	newStateMask = (ptStateMask) malloc(sizeof(tStateMask));
	...
	if(cname && cexp) {
		context = (ptContext) malloc(sizeof(tContext));
		context->name = cname;
	...
	while(variable) {
		if(variable->type == T_CHAN) {
			...
			state->chanRefs = (ptChannelRef) malloc(sizeof(tChannelRef));
...
void stateAddNever(ptState state, ptSymTabNode neverSymTab) {
	state->never = (ptStateMask) malloc(sizeof(tStateMask));
...
void stateDestroy(ptState state, byte keepPayloadAndFeatures) {
	if(state) {
		...
		if(state->never) free(state->never);
			...
			if (currentSTN->type == T_UTYPE) 	 printfTypedefState(mtypes, varName, printedSomething, state, diffState, stateMask, currentSTN->utype->child, currentSTN, i, processOffset, realOffset);
				...
				if(!diffState || value != stateGetValue(diffState->payload, processOffset+realOffset, currentSTN->type)){
					*printedSomething = 1;
                    ...
                    else { 							printf("   %-35s = %d (size: %u, offset: %u)\n", varName, value, currentSTN->memSize, processOffset+realOffset);
                    }
				}
			}
		}
...
void printChanState(ptMTypeNode mtypes, char varNameParent[200], byte* printedSomething, ptState state, ptState diffState, ptStateMask stateMask, ptSymTabNode channel, int varIndex, int processOffset) {
    ...
    sprintf(varNameBase, "%s.s%02d.f%02d", varNameParent, 0, 0);
    printf("   %-35s = N/A (size: %u, offset: %u)\n", channel->name, channel->memSize, channelOffset);
					...
					if(!diffState || value != stateGetValue(diffState->payload, realOffset, symbol->type)) {
						*printedSomething = 1;
						if(symbol->type == T_MTYPE) printf("   %-35s = %s\n", varName, getMTypeName(mtypes, value));
                            ...
                            printf("   %-35s = %d (size: %u, offset: %u)\n", varName, value, symbol->memSize, processOffset+realOffset);
                        }
					}
				}
			}
...
void printState(ptMTypeNode mtypes, ptState state, ptState diffState) {
	...
	if(trace == FULL_TRACE || !diffState) printFeature = 1;
		...
		if(!state->features && !diffState->features) printFeature = 0;
		else if(state->features && !diffState->features) printFeature = 1;
		else if(!state->features && diffState->features) printFeature = 1;
		else {
			ptBoolFct diffNeg = negateBool(diffState->features);
			ptBoolFct conjunct = addConjunction(diffNeg, state->features, 0, 1);
	...
	if(printFeature) {
		printedSomething = 1;
		printf("   %-35s = ", "features");
		if(state->features) printBool(state->features);
		else printf("/");
		printf("\n");
	}
    // time
					...
					if (currentSTN->type == T_UTYPE)	 printfTypedefState(mtypes, varName, &printedSomething, state, currentDiffSM ? diffState : NULL, currentSM, currentSTN->utype->child, currentSTN, i, currentSM->offset, currentSTN->memOffset);
						...
						if(!currentDiffSM || value != stateGetValue(diffState->payload, offset, currentSTN->type)) {
							...
							if(currentSTN->type == T_MTYPE) printf("   %-35s = %s\n", varName, getMTypeName(mtypes, value));
                                ...
                                printf("   %-35s = %d (size: %u; offset: %u)\n", varName, value, currentSTN->memSize, offset);
                            }
						}
					}
				}
...
ptStateMask stateMasksDuplicate(ptStateMask mask) {
	if(!mask) return NULL;
	ptStateMask copyMask = (ptStateMask) malloc(sizeof(tStateMask));
	...
	if(mask->context) {
		copyMask->context = (ptContext) malloc(sizeof(tContext));
...
ptChannelRef chanRefsDuplicate(ptChannelRef chanref) {
	...
	ptChannelRef newRef = (ptChannelRef) malloc(sizeof(tChannelRef));
...
ptState stateDuplicate(ptState state) {
	...
	ptState copy = (ptState) malloc(sizeof(tState));
    ...
    copy->cost = state->cost;
    copy->qualityLoss = state->qualityLoss;
...
byte stateCompare(ptState s1, void* s2Payload, ptBoolFct s2Features, int s2Time, int s2Cost, int s2QualityLoss) {
	...
	//  s1 -> s2 and s1.time >= s2.Time
	//
	// If this holds, then s1 is reachable in less products and not faster, which means
	//                that it can be considered as visited.
	// It not,        then s1 is reachable by at least one product that
	//                either was not previously explored or faster than before,
    //                so it contains some fresh info, and exploration must continue.
    
	// Convention: NULL means 'true'.
    
    if (s2Time > s1->time || s2Cost > s1->cost || s2QualityLoss > s1->qualityLoss) return STATES_SAME_S1_FRESH; // fresh because faster
    
    // From here on, s2 is not faster than s2
