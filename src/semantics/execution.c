...
#ifdef PROFILE_EXEC
	PROFILER_REGISTER(pr_executables, "executables", 200);
	PROFILER_REGISTER(pr_apply, "apply", 100);
	PROFILER_REGISTER(pr_eval, "eval", 10);
	PROFILER_REGISTER(pr_checkEqualProps, "checkEqualProps", 100);
#endif

#define DYNAMIC_CHECK if(dynamicF && expression->iVal) 	{															\
			*dynamicF = computeBoolFct(globalSymTab, mtypes, state, process, expression, flag, &foundFeature);		\
		if(!foundFeature) {																		\
			destroyBool(*dynamicF);																\
			*dynamicF = NULL;																	\
		}																						\
		else {																					\
			return 1;																			\
...
void channelPoll(ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState state, ptStateMask process, ptExpNode expression) {
	...
	while(messageFields) {
		...
		if(arg->type == E_RARG_VAR && (symbol)) {
			...
			if (channel->capacity == 0) values = readValues(_handshake_transit, chanOffset, messageFields->memSize);
			else values = readValues(state->payload, chanOffset, messageFields->memSize);
            //printf("[channelPoll] Reading %d bytes at offset %u (channel %s)\n", messageFields->memSize, chanOffset, channel->name);
            //printf("[channelPoll] Storing %d bytes at offset %u (in payload)\n", symbol->memSize, argOffset);
			storeValues(state->payload, argOffset, messageFields->memSize, values);
			free(values);
		}
...
void channelSend(ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState state, ptStateMask process, ptExpNode expression) {
    ...
    //printSymTab(messageFields, 0, channel->name);
	while(messageFields) {
		...
		if(var) {
			...
			if (messageFields->type == T_CID || messageFields->type == T_CHAN) {
                ...
                //printf("[channelSend] Reading %d bytes at offset %u (in payload)\n", messageFields->memSize, varOffset);
                //printf("[channelSend] Storing %d bytes at offset %u (channel %s)\n", messageFields->memSize, offset, channel->name);
			...
			if(channel->capacity == 0) stateSetValue(_handshake_transit, offset, messageFields->type, eval(globalSymTab, mtypes, state, process, arg, EVAL_EXPRESSION, NULL));
			else stateSetValue(state->payload, offset, messageFields->type, eval(globalSymTab, mtypes, state, process, arg, EVAL_EXPRESSION, NULL));
		}
...
ptBoolFct computeBoolFct(ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState state, ptStateMask process, ptExpNode expression, byte flag, byte * foundFeature) {
		if(!state || !process || !expression)
			return NULL;
#ifdef DEBUGEXEC
		printf("[computeBoolFct] Expression (%s) at line %d\n", getExpTypeName(expression->type), expression->lineNb);
#endif
		ptBoolFct addedFeatures_ = NULL;
		...
		if(flag == EVAL_EXECUTABILITY && handshake != NO_HANDSHAKE && expression->type != E_STMNT_CHAN_RCV) return NULL;
		else {
			switch(expression->type) {
				...
				case(E_EXPR_NE):
					...
					if(expression->children[0]->type == E_EXPR_COUNT) {
						...
						countValue = eval(globalSymTab, mtypes, state, process, expression->children[1], flag, NULL);
						...
						switch(expression->type) {
							...
							case(E_EXPR_LT):
								if(countValue <= 0)
									...
									return negateBool(createVariable(varName));
								}
								else result.boolFctResult = getTrue();
							...
							case(E_EXPR_EQ):
								if(countValue < 0 || countValue > result.symbol->bound)
									...
									snprintf(varName, sizeof(varName), "%s[%d]%s", result.symbol->name, countValue, suffix);
									formula = addConjunction(formula, negateBool(createVariable(varName)), 0, 0);
								}
							case(E_EXPR_NE):
								if(countValue < 0 || countValue > result.symbol->bound)
									return getTrue();
									...
									formula = createVariable(varName);
									snprintf(varName, sizeof(varName), "%s[%d]%s", result.symbol->name, countValue, suffix);
									formula = negateBool(addConjunction(formula, negateBool(createVariable(varName)), 0, 0));
								}
						}
					}
					else return NULL;
				...
				case(E_EXPR_OR):
					...
					*foundFeature = foundSubFeature1 || foundSubFeature2;
					return !(*foundFeature) ? NULL : (expression->type == E_EXPR_AND) ? addConjunction(formula, formula2, 0, 0) : addDisjunction(formula, formula2, 0, 0);


				case(E_EXPR_NEG):
					formula = computeBoolFct(globalSymTab, mtypes, state, process, expression->children[0], flag, foundFeature);
					return *foundFeature ? negateBool(formula) : NULL;
					...
					if(feature->type != T_FEAT && feature->type != T_UFEAT) {
						...
						*foundFeature = 1;
						...
						if(type == T_BOOL || type == T_FEAT || type == T_UFEAT || type == T_UTYPE) {
							ptBoolFct res = createVariable(path);
							free(path);
							return res;
						}
						else if(type == T_INT) {
				...
				default:
					if(foundFeature)
						*foundFeature = 0;
					return NULL;//failure("[computeBoolFct] Unexpected expression in Boolean function at line %d\n", expression->lineNb);
			}
		}
...
static byte _else = 0;
int eval(ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState state, ptStateMask process, ptExpNode expression, byte flag, ptBoolFct * dynamicF) {
	...
	if(flag == EVAL_EXECUTABILITY && handshake != NO_HANDSHAKE && expression->type != E_STMNT_CHAN_RCV) returnValue = 0;
		...
		switch(expression->type) {
				...
				if (channelIsEmpty(globalSymTab, mtypes, state, process, expression->children[0]))
					...
					while(currentArg && !stop) {
						...
						if(currentArg->children[0]->type != E_RARG_VAR) {
							...
							if (eval(globalSymTab, mtypes, state, process, currentArg->children[0], flag, NULL) != sendValue) {
								returnValue = 0;
								stop = 1;
							}
						} else {
							...
							if (!symbol) {
								...
								returnValue = eval(globalSymTab, mtypes, state, process, currentArg->children[0], EVAL_EXPRESSION, NULL);
            ...
            case(E_STMNT_COST): // return value is the delay value
            case(E_STMNT_QUALITY):
                if(flag == EVAL_EXECUTABILITY) returnValue = 1;
                else returnValue = expression->iVal;
                break;
            
            case(E_STMNT_WHEN): // return value is the delay value
            case(E_STMNT_INCR):
            case(E_STMNT_DECR):
                if(flag == EVAL_EXECUTABILITY) returnValue = 1;
                else returnValue = eval(globalSymTab, mtypes, state, process, expression->children[0], flag, NULL);
                break;
			...
			case(E_EXPR_PLUS):
				returnValue = eval(globalSymTab, mtypes, state, process, expression->children[0], flag, NULL) + eval(globalSymTab, mtypes, state, process, expression->children[1], flag, NULL);
				break;
			...
			case(E_EXPR_UMIN):
				returnValue = - eval(globalSymTab, mtypes, state, process, expression->children[0], flag, NULL);
				break;
			...
			case(E_EXPR_COND):
				if(eval(globalSymTab, mtypes, state, process, expression->children[0], EVAL_EXPRESSION, NULL) > 0)
					returnValue = eval(globalSymTab, mtypes, state, process, expression->children[1], flag, NULL);
				else
					returnValue = eval(globalSymTab, mtypes, state, process, expression->children[2], flag, NULL);
				break;
			...
			case(E_VARREF_NAME):
				...
				if(symbol) {
					...
					return stateGetValue(state->payload, offset, field->type);
...
int priority(ptExpNode expression) {
    if (!expression) return 0;
    
    switch(expression->type) {
        case(E_STMNT_NF):
            return priority(expression->children[0]) + priority(expression->children[1]);
            break;
            
        case(E_STMNT_COST):
            return -costFunction(0, expression->iVal, 0);
            
        case(E_STMNT_QUALITY):
            return -costFunction(0, 0, expression->iVal);
...
byte checkEqualProps(ptList props, ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState s, ptList props2, ptSymTabNode globalSymTab2, ptMTypeNode mtypes2, ptState s2) {
	...
	while(tmp && tmp2) {
		...
		byte eval1 = eval(globalSymTab, mtypes, s, s->mask, prop, EVAL_EXECUTABILITY, NULL);
		byte eval2 = eval(globalSymTab2, mtypes2, s2, s2->mask, prop2, EVAL_EXECUTABILITY, NULL);
		if(eval1 != eval2) {
#ifdef PROFILE_EXEC
...
void updateDelay(ptProcessTransition newT, ptList prevTs, int timeElapsed) {
    ptList currentPrev = prevTs;
    while (currentPrev) {
        ptProcessTransition prevT = (ptProcessTransition) currentPrev->value;
        
        if (newT->trans == prevT->trans) {
            //printf("[execution] Delay of transition TL%03d changed from %d to %d\n", newT->trans->lineNb, newT->delay, prevT->delay);
            newT->delay = prevT->delay;
            currentPrev = NULL;
        } else {
...
int partition(ptList * list, int l, int r) {
    
    if (l == r) return l;
    
    //printf("Partitioning list of size %d, from %d to %d\n", listCount(*list), l, r);
    ptList pivot = listGet(*list, r);
    if (!pivot) failure("pivot is NULL\n");
    if (!pivot->value) failure("pivot->value is NULL\n");
    if (!((ptProcessTransition) pivot->value)->trans) failure("(ptProcessTransition) pivot->value)->trans is NULL\n");
    if (!((ptProcessTransition) pivot->value)->trans->expression) failure("(ptProcessTransition) pivot->value)->trans->expression is NULL\n");
    int pVal = priority(((ptProcessTransition) pivot->value)->trans->expression);
    //printf("Pivot is %d, value = %d\n", r, pVal);
    
    int i = l - 1;
    
    for (int j = i + 1; j < r; j++) {
        ptList higher = listGet(*list, j);
        int hVal = priority(((ptProcessTransition) higher->value)->trans->expression);
        
        if (hVal >= pVal) {
            i++;
            //printf("Swap %d (value = %d) with %d (value = %d)\n", j, hVal, i);
            //listPrint(*list);
            *list = listSwap(*list, i, j);
            //listPrint(*list);
        }
    }
    
    //listPrint(*list);
    //printf("Swap pivot %d (value = %d) with %d\n", r, pVal, i + 1);
    *list = listSwap(*list, i + 1, r);
    //listPrint(*list);
    //printf("end of partitioning\n");
    return (i + 1);
}

void quickSort(ptList * list, int l, int r) {
...
void orderByPriority(ptList * list) {
    if (!(*list)) return;
    //printf("<---- Starting ordering\n");
    quickSort(list, 0, listCount(*list) - 1);
...
ptList executables(ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState state, byte stutter, int nbErrors, byte * hasDeadlock, ptBoolFct* featuresOut, byte* allProductsOut, ptBoolFct* noOutgoing, byte* resetExclusivity, ptList previousTrans, int previousDelay) {
    ...
    int smallestDelay = INT_MAX;
    byte resetExclusivityDelay = false;
...
OneTrans:	while(currentTrans) {
				if(eval(globalSymTab, mtypes, state, maskPtr, ((ptFsmTrans)currentTrans->value)->expression, EVAL_EXECUTABILITY, NULL) > 0) {
					...
					ptBoolFct conjunct = addConjunction(currentFeatures, state->features, 1, 1);
					...
					if((stateGetValue(state->payload, OFFSET_HANDSHAKE, T_INT) != NO_HANDSHAKE) && (((ptFsmTrans)currentTrans->value)->expression->type == E_STMNT_CHAN_SND)) {
						...
						ptList e_ = executables(globalSymTab, mtypes, state, 0, nbErrors, &responseHasDeadlock, featuresOut, allProductsOut, noOutgoing, NULL, previousTrans, previousDelay);
						...
						while(response) {
                            ...
                            // time
                            ableTrans->delay = ableTrans->trans->delay ? eval(globalSymTab, mtypes, state, maskPtr, ableTrans->trans->delay, EVAL_EXPRESSION, NULL) : 0;
                            
                            updateDelay(ableTrans, previousTrans, previousDelay);
                            
                            if (ableTrans->delay < smallestDelay) {
                                smallestDelay = ableTrans->delay;
                            }
						...
						if(!(currentFeatures) || isSatisfiableWrtFD(conjunct)) {
                            ...
                            if(ableTrans->response) {
                                if (ableTrans->response->delay > ableTrans->delay) {
                                    ableTrans->delay = ableTrans->response->delay;
                                }
                                ableTrans->response->delay = 0;
                            }
	...
	if (stateGetValue(state->payload, OFFSET_EXCLUSIVE, T_BYTE) != NO_PROCESS) {
        if (!noDeadlock || smallestDelay > 0) {
            //printf("reset exclusitivity (was %d); smallestdelay = %d\n", stateGetValue(state->payload, OFFSET_EXCLUSIVE, T_BYTE), smallestDelay);
	...
	if(freeAllOut) free(allProductsOut);
#ifdef DEBUGEXEC
	printf("[/Executables]\n\n\n");
#endif
    
    // time
	//return removeSlowerTransitions(E,smallestDelay);
    
    if (prior) orderByPriority(&E);
...
ptList executablesNonStutter(ptList props, ptSymTabNode globalSymTab, ptMTypeNode mtypes, ptState state, byte stutter, int nbErrors, byte* hasDeadlock, ptBoolFct* featuresOut, byte* allProductsOut, ptBoolFct* noOutgoing) {
	...
	stateCopy->features = copyBool(state->features);
	ptStackElt elt = createStackElement(stateCopy,0);
	elt->E = executables(globalSymTab, mtypes, stateCopy, stutter, nbErrors, hasDeadlock, featuresOut, allProductsOut, noOutgoing, &resetExclusivity, NULL, 0);
	if(resetExclusivity) {
		stateSetValue(state->payload, OFFSET_EXCLUSIVE, T_BYTE, NO_PROCESS);
	...
	*newKey = stateCopy->payloadHash;
	...
	while(!empty(stack)) {
		...
		if(!current->E) {
			...
			E = executables(globalSymTab, mtypes, s_, stutter, nbErrors, hasDeadlock, featuresOut, allProductsOut, noOutgoing, &resetExclusivity, NULL, 0);
			if(resetExclusivity) {
			...
			if(!foundVisited) {
				stackFeatures = push(stackFeatures, addConjunction(top(stackFeatures), currentTrans->features, 1, 1));
				...
				*newKey = s_Copy->payloadHash;
				...
				if(foundCandidate) {
					destroyProcTransList(E, processTrans);
					E = NULL;
					ptDirectTransition dtrans = (ptDirectTransition) malloc(sizeof(tDirectTransition));
					dtrans->target = s_;
					dtrans->features = (ptBoolFct) top(stackFeatures);
					pop(&stackFeatures);
...
Eval:	transList = neverNode->trans;
		while(transList) {
			if(eval(globalSymTab, mtypes, state, state->never, ((ptFsmTrans)(transList->value))->expression, EVAL_EXECUTABILITY, NULL)) {
...
ptProcessTransition createProcessTransition(ptStateMask process, ptFsmTrans trans, ptBoolFct features, ptProcessTransition response) {
	ptProcessTransition ptrans = (ptProcessTransition) malloc(sizeof(tProcessTransition));
...
void destroyProcTransList(ptList list, byte process_or_direct) {
	...
	while(list) {
		...
		if(process_or_direct == directTrans) {
			...
			free((ptDirectTransition) list->value);
		}
		else {
...
void executePromelaPrint(ptSymTabNode globalSymTab, ptMTypeNode mtypes, char * message, ptExpNode argNode, ptState state, ptStateMask process) {
	...
	int i = 0;
	while(currentArg) {
		argi[i] = eval(globalSymTab, mtypes, state, process, currentArg->children[0], EVAL_EXPRESSION, NULL);
...
Apply:
	...
	switch(expression->type) {
		...
		case(E_STMNT_CHAN_SND):
			...
			if(channel->capacity == 0) {
                ...
                // time
                /*if (procTrans->trans->delay > procTrans->response->trans->delay) {
                    // time partly elapsed in copy already
                    timeElapsed = procTrans->trans->delay - procTrans->response->trans->delay;
                } else {
                    timeElapsed = 0; // time already elapsed in copy
                }*/
			}
			break;
		...
		case(E_STMNT_ASGN):
			...
			else stateSetValue(copy->payload, offset, field->type, eval(globalSymTab, mtypes, copy, process, expression->children[1], EVAL_EXPRESSION, NULL));
			break;
		...
		case(E_EXPR_RUN):
			...
			if(copy->nbProcesses > MAX_PROCESS) failure("Cannot instantiate more than %d processes.", MAX_PROCESS);
				...
				if(expression->children[1]) {
					failure("[eval] Feature contexts are not allowed in this variant of ProVeLines.\n");
				}
				else
				...
				while (argList) {
					...
					else 	stateSetValue(copy->payload, offset, param->type, eval(globalSymTab, mtypes, copy, process, arg, EVAL_EXPRESSION, NULL));
					argList = argList->children[1];
					param = param->next;
				}
        ...
        case(E_STMNT_COST):
            copy->cost = copy->cost + eval(globalSymTab, mtypes, state, process, expression, EVAL_EXPRESSION, NULL);
            break;
        case(E_STMNT_QUALITY):
            copy->qualityLoss = copy->qualityLoss + eval(globalSymTab, mtypes, state, process, expression, EVAL_EXPRESSION, NULL);
            break;
            
        case(E_STMNT_NF):
            if (expression->children[0]->type == E_STMNT_COST) {
                copy->cost = copy->cost + eval(globalSymTab, mtypes, state, process, expression->children[0], EVAL_EXPRESSION, NULL);
            } else if (expression->children[0]->type == E_STMNT_QUALITY) {
                copy->qualityLoss = copy->qualityLoss + eval(globalSymTab, mtypes, state, process, expression->children[0], EVAL_EXPRESSION, NULL);
            }
            if (expression->children[1] && expression->children[1]->type == E_STMNT_COST) {
                copy->cost = copy->cost + eval(globalSymTab, mtypes, state, process, expression->children[1], EVAL_EXPRESSION, NULL);
            } else if (expression->children[1] && expression->children[1]->type == E_STMNT_QUALITY) {
                copy->qualityLoss = copy->qualityLoss + eval(globalSymTab, mtypes, state, process, expression->children[1], EVAL_EXPRESSION, NULL);
            }
            break;
	}
	...
	if(!leaveUntouched) {
		...
		copy->features = copyBool(procTrans->features);
        
		// Proceed in automaton
		storeNodePointer(copy, maskLookup(copy, procTrans->process->pid), procTrans->trans->target);

		// Set exclusivity of process
        if(procTrans->trans->target && (procTrans->trans->target->flags & N_ATOMIC) == N_ATOMIC) {
            stateSetValue(copy->payload, OFFSET_EXCLUSIVE, T_BYTE, process->pid);
        }
        else {
            stateSetValue(copy->payload, OFFSET_EXCLUSIVE, T_BYTE, NO_PROCESS);
        }
    ...
    // time
    copy->time = copy->time + procTrans->delay;
...
ptStackElt createStackElement(ptState state, int nbErrors) {
	ptStackElt elt = (ptStackElt) malloc(sizeof(tStackElt));
...
void destroyStackElement(ptStackElt elt, byte process_or_direct) {
	if(elt) {
		...
		if(elt->state) stateDestroy(elt->state, true);
        
        if (elt->onHold) {
            ptList onHold = elt->onHold;
            while(onHold) {
                destroyProcessTransition(onHold->value);
                onHold = onHold->next;
            }
            listDestroy(elt->onHold);
        }
		free(elt);
	}
}
...
void launchExecution(ptSymTabNode globalSymTab, ptMTypeNode mtypes) {
	...
	ptList E, E_;
	E = executables(globalSymTab, mtypes, state, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, 0);
	while(E) {
